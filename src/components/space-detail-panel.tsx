    ...
    <AnimatePresence>
      {open && space ? (
        <motion.aside
          ...
          role="dialog"
          aria-modal="false"
          aria-label={`Detalhes de ${space.name}`}
          initial={{ opacity: 0, x: 28 }}
          animate={{ opacity: 1, x: 0 }}
          exit={{ opacity: 0, x: 36 }}
          transition={{ type: "spring", stiffness: 340, damping: 32 }}
          className={cn(
            "absolute z-30 flex flex-col overflow-hidden border border-stone-200 bg-white shadow-2xl",
            "bottom-[42dvh] left-0 right-0 top-0 rounded-none md:top-20 md:bottom-4 md:rounded-2xl",
            listOpen
          ...
          <div className="absolute inset-x-0 bottom-0 border-t border-border/70 bg-white/95 p-3 backdrop-blur-sm md:p-4">
            ...
            {!onRequest ? (
              <p className="mt-1.5 text-center text-xs text-muted-foreground">
                Pedido de reserva na próxima etapa
              </p>
            ) : null}
          </div>
        </motion.aside>
