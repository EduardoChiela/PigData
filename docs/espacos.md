...
- Detalhe (T03) no mapa: painel lateral em `/` (`SpaceDetailPanel`) com galeria, perfil completo e CTA de pedido. Clique na foto abre **lightbox em tela cheia** com carrossel (setas, teclado ←/→, Escape, miniaturas).
...
- Homologação: organizador em `/painel-acit` (Filiados) altera status para `verificado` / `recusado`; Meus anúncios reflete o selo; listing `verificado` entra no mapa (camada A).
